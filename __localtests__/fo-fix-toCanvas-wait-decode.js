/**
 * Lab fork: extra decode / rAF waits before drawImage (settle probe).
 * Opt in: rasterPatch: 'lab-toCanvas-wait-decode' → __localtests__/fo-fix-toCanvas-wait-decode.js
 */
import { isSafari } from '../src/utils/browser.js'

/** modern-screenshot drawImageInterval default (ms) — upstream structural default. */
...
export async function toCanvas(url, options) {
  ...
  const passes = Math.max(1, Math.floor(decodePasses))
