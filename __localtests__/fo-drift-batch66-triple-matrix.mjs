 ...
 *
 *   node __localtests__/fo-drift-batch66-triple-matrix.mjs
 *   node __localtests__/fo-drift-batch66-triple-matrix.mjs --parallel 4 --landmark Home
...
const DEFAULT_IDS = ['product-baseline', 'drift-batch66-*']
...
function spawnWorker(chunkIndex, port, ids) {
  return new Promise((resolve, reject) => {
    const outPath = path.join(SANDBOX, `fo-drift-batch66-chunk-${chunkIndex}.json`)
    const childArgs = [
...
async function workerMain() {
  ...
  const outPath = mergeOutArg
    ...
    : path.join(SANDBOX, `fo-drift-batch66-chunk-${workerChunk ?? 0}.json`)
...
async function mainMain() {
  ...
  if (!recipeIds.length) {
    console.error('No drift-batch66 recipes found — add recipes-drift-batch66-triple.js shard')
    process.exit(1)
  }

  console.log(
    `Drift batch 66 triple: ${recipeIds.length} recipes landmark=${landmark} parallel=${parallel}`,
  )
  ...
  const reportPath = path.join(SANDBOX, 'fo-drift-batch66-triple-matrix.json')
  ...
  fs.writeFileSync(
    path.join(ARTIFACTS, 'fo-drift-batch66-triple-matrix.json'),
