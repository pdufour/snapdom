#!/usr/bin/env node
/**
 * Drift batch 1 matrix — headed Chrome, optional parallel workers, rank by |svgΔ|+|canvasΔ|.
 *
 *   node __localtests__/fo-drift-batch1-matrix.mjs
 *   node __localtests__/fo-drift-batch1-matrix.mjs --parallel 4 --landmark Home
  ...
  for (const r of ok) {
    console.log(
      r.recipeId.padEnd(44) +
        fmtInkPx(r.sumAbsSvgCanvas).padStart(9) +
...
function spawnWorker(chunkIndex, port, ids) {
  return new Promise((resolve, reject) => {
    const outPath = path.join(SANDBOX, `fo-drift-batch1-chunk-${chunkIndex}.json`)
    const childArgs = [
...
async function workerMain() {
  ...
  const outPath = mergeOutArg
    ...
    : path.join(SANDBOX, `fo-drift-batch1-chunk-${workerChunk ?? 0}.json`)
...
async function mainMain() {
  ...
  if (!recipeIds.length) {
    console.error('No drift-batch1 recipes found — run after adding recipes-drift-batch1.js shard')
    process.exit(1)
  }

  console.log(
    `Drift batch 1: ${recipeIds.length} recipes landmark=${landmark} parallel=${parallel}`,
  )
  ...
  const best = ranked.reduce(
    (a, b) => (b.sumAbsSvgCanvas < a.sumAbsSvgCanvas ? b : a),
    ranked[0] ?? null,
  )

  const reportPath = path.join(SANDBOX, 'fo-drift-batch1-matrix.json')
  ...
  const report = {
    ...
    matrixIds,
    recipeIds,
    rankMetric: '|svgΔ|+|canvasΔ|',
    baselineSumAbs: baseline?.sumAbsSvgCanvas ?? null,
    bestRecipeId: best?.recipeId ?? null,
    bestSumAbs: best?.sumAbsSvgCanvas ?? null,
    ranked,
    errors: allScored.filter((r) => r.error),
  }
  fs.writeFileSync(reportPath, `${JSON.stringify(report, null, 2)}\n`)
  fs.writeFileSync(
    path.join(ARTIFACTS, 'fo-drift-batch1-matrix.json'),
    `${JSON.stringify(report, null, 2)}\n`,
  )
  console.log(`\nWrote ${reportPath}`)
  if (best) {
    console.log(
      `Best |svgΔ|+|canvasΔ|: ${best.recipeId} ${fmtInkPx(best.sumAbsSvgCanvas)} (baseline ${fmtInkPx(baseline?.sumAbsSvgCanvas ?? NaN)})`,
    )
  }
}
