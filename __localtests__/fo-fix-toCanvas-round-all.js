/**
 * Lab fork: round every layout/draw dimension before blit (grid-snap probe).
 * Opt in: rasterPatch: 'lab-toCanvas-round-all' → __localtests__/fo-fix-toCanvas-round-all.js
 */
...
export async function toCanvas(url, options) {
  ...
  if (hasW && hasH) {
    outW = Math.max(1, roundDim(optW))
    outH = Math.max(1, roundDim(optH))
  } else if (hasW) {
    const k = roundDim(optW) / Math.max(1, refW)
    outW = roundDim(optW)
    outH = roundDim(refH * k)
  } else if (hasH) {
    const k = roundDim(optH) / Math.max(1, refH)
    outH = roundDim(optH)
    outW = roundDim(refW * k)
  } else {
    outW = roundDim(natW)
    outH = roundDim(natH)
  }

  outW = roundDim(outW * scale)
  outH = roundDim(outH * scale)

  const canvas = document.createElement('canvas')
  const pxW = Math.max(1, roundDim(outW * dpr))
  const pxH = Math.max(1, roundDim(outH * dpr))
  canvas.width = pxW
  canvas.height = pxH
