#!/usr/bin/env node
/**
 * Headed matrix: raster-only SVG fork wave-2 (6 patches + product-baseline).
 *
 * SVG leg uses original capture; canvas uses patched fork at decode only (lab-toCanvas-decode).
 *
 *   npm run compile && npm run debug:raster-fork-w2-matrix
 *   node __localtests__/fo-raster-fork-wave2-matrix.mjs --json .sandbox-edit/raster-fork-wave2.json
 *
...
import { classifyParityStage, PARITY_STAGE_TOL_PX } from './fo-fix-lab.mjs'

const __dirname = path.dirname(fileURLToPath(import.meta.url))
const REPO_ROOT = path.join(__dirname, '..')
const DEFAULT_PORT = 8787
const DEFAULT_OUT = path.join(REPO_ROOT, '.sandbox-edit', 'raster-fork-wave2.json')

const FORK_RECIPE_IDS = [
  'tc-fork-w2-leading-trim-inject',
  'tc-fork-w2-align-self-flex-start',
  'tc-fork-w2-line-height-normal-important',
  'tc-fork-w2-svg-root-translate-y-minus-half-leading',
  'tc-fork-w2-foreignObject-overflow-hidden',
  'tc-fork-w2-combo-lh-normal-flex-start',
]
...
function summarize(rows) {
  ...
  const forkRows = rows.filter((r) => r.recipeId.startsWith('tc-fork-w2-'))
  ...
  try {
    ...
    const payload = {
      ...
      rows: rows.map((row) => ({
        ...
        rasterOnlySvgPatch: row.recipeId.startsWith('tc-fork-w2-')
          ? row.recipeId.replace(/^tc-fork-w2-/, '')
          : null,
      })),
      summary,
    }
