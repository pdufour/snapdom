#!/usr/bin/env node
/**
 * Drift batch 777 triple matrix — headed Chrome, optional parallel workers.
 * Rank by tripleScore = |svgΔ| + |canvasΔ| + |svg−canvas|.
 *
 *   node __localtests__/fo-drift-batch777-triple-matrix.mjs
 *   node __localtests__/fo-drift-batch777-triple-matrix.mjs --parallel 4 --landmark Home
...
const DEFAULT_IDS = ['product-baseline', 'drift-batch777-*']
...
function spawnWorker(chunkIndex, port, ids) {
  return new Promise((resolve, reject) => {
    const outPath = path.join(SANDBOX, `fo-drift-batch777-chunk-${chunkIndex}.json`)
    const childArgs = [
...
async function workerMain() {
  ...
  const outPath = mergeOutArg
    ...
    : path.join(SANDBOX, `fo-drift-batch777-chunk-${workerChunk ?? 0}.json`)
...
async function mainMain() {
  ...
  if (!recipeIds.length) {
    console.error('No drift-batch777 recipes found — add recipes-drift-batch777-triple.js shard')
    process.exit(1)
  }

  console.log(
    `Drift batch 777 triple: ${recipeIds.length} recipes landmark=${landmark} parallel=${parallel}`,
  )
  ...
  const reportPath = path.join(SANDBOX, 'fo-drift-batch777-triple-matrix.json')
  ...
  fs.writeFileSync(
    path.join(ARTIFACTS, 'fo-drift-batch777-triple-matrix.json'),
