#!/usr/bin/env node
/**
 * Drift batch 2 triple matrix — headed Chrome, optional parallel workers.
 * Rank by tripleScore = |svgΔ| + |canvasΔ| + |svg−canvas|.
 *
 *   node __localtests__/fo-drift-batch32222-triple-matrix.mjs
 *   node __localtests__/fo-drift-batch32222-triple-matrix.mjs --parallel 4 --landmark Home
...
const DEFAULT_IDS = ['product-baseline', 'drift-batch32222-*']
const BASE_PORT = 8860
...
function spawnWorker(chunkIndex, port, ids) {
  return new Promise((resolve, reject) => {
    const outPath = path.join(SANDBOX, `fo-drift-batch32222-chunk-${chunkIndex}.json`)
    const childArgs = [
...
async function workerMain() {
  ...
  const outPath = mergeOutArg
    ...
    : path.join(SANDBOX, `fo-drift-batch32222-chunk-${workerChunk ?? 0}.json`)
...
async function mainMain() {
  ...
  if (!recipeIds.length) {
    console.error('No drift-batch32222 recipes found — add recipes-drift-batch32222-triple.js shard')
    process.exit(1)
  }

  console.log(
    `Drift batch 2 triple: ${recipeIds.length} recipes landmark=${landmark} parallel=${parallel}`,
  )
  ...
  const reportPath = path.join(SANDBOX, 'fo-drift-batch32222-triple-matrix.json')
  ...
  fs.writeFileSync(
    path.join(ARTIFACTS, 'fo-drift-batch32222-triple-matrix.json'),
