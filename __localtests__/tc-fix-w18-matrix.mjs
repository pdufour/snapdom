#!/usr/bin/env node
/**
 * Wave-18 matrix calibrate — refine w7 −0.203 px.
 * Writes `.sandbox-edit/tc-fix-w18-matrix.json`.
 *
 *   npm run debug:tc-fix-w18-matrix-calibrate
 *   node __localtests__/tc-fix-w18-matrix.mjs --json .sandbox-edit/tc-fix-w18-matrix.json
 */
...
const DEFAULT_OUT = path.join(REPO_ROOT, '.sandbox-edit', 'tc-fix-w18-matrix.json')
...
const RECIPE_IDS_CSV =
  'product-baseline,tc-fix-w7-rfork-fo-y-half-leading-meta,tc-fix-w18-*'
...
async function main() {
  ...
  const w18Ids = Object.keys(byRecipe).filter((id) => id.startsWith('tc-fix-w18-'))
  const w18Sorted = w18Ids
  ...
  const bestW18 = w18Sorted[0] ?? null
  const gatePass = w18Sorted.some(
    (r) => r.pass && (r.worstAbsDeltaTop ?? 99) <= GATE_PX,
  )
  const beatsW7 =
    bestW18 != null &&
    Number.isFinite(bestW18.worstAbsDeltaTop) &&
    bestW18.worstAbsDeltaTop < W7_BEST_PX
  ...
  const payload = {
    ...
    note: gatePass
      ? 'At least one w18 row passes ≤0.06 px gate on Home+Products.'
      : beatsW7
        ? `Best w18 beats w7 ${W7_BEST_PX}px but does not pass ≤${GATE_PX}px gate.`
        : 'No w18 row beats w7 0.203 px; residual likely Range subpixel vs integer scan.',
    reference: {
      baseline: { recipeId: BASELINE, ...baseline },
      w7: { recipeId: W7_REF, ...w7 },
    },
    bestW18,
    w18Leaders: w18Sorted.slice(0, 8),
    w18All: w18Sorted,
    rows: calibrateRows,
  }
  ...
  console.log('\n--- Wave-18 calibrate (Home + Products @ dpr=1) ---\n')
  console.log(
    'recipe'.padEnd(56) +
      'worst|Δ|'.padStart(10) +
      'mean|Δ|'.padStart(10) +
  ...
  for (const row of w18Sorted) {
    console.log(
      row.recipeId.padEnd(56) +
  ...
  console.log(
    `Best w18: ${bestW18?.recipeId ?? '—'} (worst|Δ|=${bestW18?.worstAbsDeltaTop?.toFixed(3) ?? '—'})`,
  )
