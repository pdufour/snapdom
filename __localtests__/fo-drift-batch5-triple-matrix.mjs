#!/usr/bin/env node
/**
 * Drift batch 5 triple matrix — headed Chrome, optional parallel workers.
 * Rank by tripleScore = |svgΔ| + |canvasΔ| + |svg−canvas|.
 *
 *   node __localtests__/fo-drift-batch5-triple-matrix.mjs
 *   node __localtests__/fo-drift-batch5-triple-matrix.mjs --parallel 4 --landmark Home
...
const DEFAULT_IDS = ['product-baseline', 'drift-batch5-*']
const BASE_PORT = 8870
...
function spawnWorker(chunkIndex, port, ids) {
  return new Promise((resolve, reject) => {
    const outPath = path.join(SANDBOX, `fo-drift-batch5-chunk-${chunkIndex}.json`)
    const childArgs = [
...
async function workerMain() {
  ...
  const outPath = mergeOutArg
    ...
    : path.join(SANDBOX, `fo-drift-batch5-chunk-${workerChunk ?? 0}.json`)
...
async function mainMain() {
  ...
  if (!recipeIds.length) {
    console.error('No drift-batch5 recipes found — add recipes-drift-batch5-triple.js shard')
    process.exit(1)
  }
  ...
  const reportPath = path.join(SANDBOX, 'fo-drift-batch5-triple-matrix.json')
  ...
  fs.writeFileSync(
    path.join(ARTIFACTS, 'fo-drift-batch5-triple-matrix.json'),
