 ...
 *
 *   node __localtests__/fo-drift-batch1-triple-matrix.mjs
 *   node __localtests__/fo-drift-batch1-triple-matrix.mjs --parallel 4 --landmark Home
 *   npm run debug:fo-drift-batch1-triple
 */
...
async function mainMain() {
  ...
  if (!recipeIds.length) {
    console.error('No drift-batch1 recipes found — add recipes-drift-batch1-triple.js shard')
    process.exit(1)
  }

  console.log(
    `Drift batch 1 triple: ${recipeIds.length} recipes landmark=${landmark} parallel=${parallel}`,
  )
  ...
  const reportPath = path.join(SANDBOX, 'fo-drift-batch1-triple-matrix.json')
  ...
  fs.writeFileSync(
    path.join(ARTIFACTS, 'fo-drift-batch1-triple-matrix.json'),
