 ...
 *
 *   node __localtests__/fo-drift-batch3-triple-matrix.mjs
 *   node __localtests__/fo-drift-batch3-triple-matrix.mjs --parallel 4 --landmark Home
...
const DEFAULT_IDS = ['product-baseline', 'drift-batch3-*']
...
function spawnWorker(chunkIndex, port, ids) {
  return new Promise((resolve, reject) => {
    const outPath = path.join(SANDBOX, `fo-drift-batch3-chunk-${chunkIndex}.json`)
    const childArgs = [
...
async function workerMain() {
  ...
  const outPath = mergeOutArg
    ...
    : path.join(SANDBOX, `fo-drift-batch3-chunk-${workerChunk ?? 0}.json`)
...
async function mainMain() {
  ...
  if (!recipeIds.length) {
    console.error('No drift-batch3 recipes found — add recipes-drift-batch3-triple.js shard')
    process.exit(1)
  }
  ...
  const reportPath = path.join(SANDBOX, 'fo-drift-batch3-triple-matrix.json')
  ...
  fs.writeFileSync(
    path.join(ARTIFACTS, 'fo-drift-batch3-triple-matrix.json'),
