#!/usr/bin/env node
/**
 * Run Home drift matrix batches (30 recipes each), optionally in parallel.
 *
 *   node __localtests__/fo-drift-batch-home-parallel.mjs --from 0 --count 4
 *   node __localtests__/fo-drift-batch-home-parallel.mjs --offsets 0,30,60,90 --parallel 3
 *   node __localtests__/fo-drift-batch-home-parallel.mjs --offsets auto --parallel 3 --include-inactive \
 *     --ids 'product-baseline,fix321-*,tc-decode-*,tc-raster-*'
 *   npm run debug:fo-drift-batch-home-parallel
 */
...
const DEFAULT_IDS =
  'product-baseline,tc-fix-*,tc-blh-*,tc-decode-*,tc-raster-*,fix321-*,tc-fork-*'
...
const count = Number(argAfter('--count') ?? 3)
...
function defaultMergeOutPath() {
  ...
  if (slug === defaultSlug.slice(0, slug.length) && matrixIds.length === defaultSlug.length) {
    return path.join(SANDBOX, 'fo-drift-batch-home-merged.json')
  }
  if (
    matrixIds.some((p) => p.startsWith('fix321-') || p.startsWith('tc-decode-') || p.startsWith('tc-raster-')) &&
    !matrixIds.some((p) => p.startsWith('tc-fix-'))
  ) {
    return path.join(SANDBOX, 'fo-drift-batch-home-decode-merged.json')
  }
  if (
    matrixIds.some((p) => p.startsWith('tc-blh-') || p === 'tc-fix-w7-*' || p === 'tc-fix-w16-*' || p === 'tc-fix-w18-*') &&
    !matrixIds.some((p) => p.startsWith('tc-decode-') || p.startsWith('tc-raster-') || p.startsWith('fix321-'))
  ) {
    return path.join(SANDBOX, 'fo-drift-batch-home-strut-merged.json')
  }
  return path.join(SANDBOX, `fo-drift-batch-home-${slug || 'custom'}-merged.json`)
}
...
function mergeReports(activeOffsets) {
  ...
  if (
    ...
    console.log(
      `\nNext: node __localtests__/fo-drift-batch-home-parallel.mjs --offsets ${next},${next + LIMIT},${next + LIMIT * 2} --parallel 3 --include-inactive --ids '${idsFlag}'`,
    )
  } else {
...
async function main() {
  ...
  console.log(
    `Home drift batches: offsets=[${active.join(',')}] parallel=${parallel} limit=${LIMIT} ids=${matrixIds.join(',')}`,
  )
  if (!active.length) {
