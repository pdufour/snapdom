...
/** Mirrors capture {@link isFlexCrossStretchItem} — debug only, not imported from src/. */
function debugIsFlexOrGridItem(el) {
...
function debugEffectiveCrossAlign(el, style) {
...
/** @param {Element} el @param {CSSStyleDeclaration} style */
function debugIsFlexCrossStretchItem(el, style) {
  if (!debugIsFlexOrGridItem(el)) return false
  const cross = debugEffectiveCrossAlign(el, style)
...
/**
 * Flex ancestry + stretch/line-box facts for a landmark (live DOM or SVG clone).
 * Used by checkout structure report and `window.__snapdomDebug`.
 * @param {Element} el
 * @param {Element} root capture root (`#capture-target` or clone equivalent)
 */
export function collectFlexLayoutDebug(el, root) {
  if (!el || el.nodeType !== 1) return null

  const cs0 = getComputedStyle(el)
  const box0 = relRect(el, root)
  const layoutLh0 = measureLayoutLineBoxPx(cs0, el)
  const lineHeightPx0 = resolveLineHeightPx(cs0, el)
  const padTB =
    (parseFloat(cs0.paddingTop) || 0) + (parseFloat(cs0.paddingBottom) || 0)
  const borderTB =
    (parseFloat(cs0.borderTopWidth) || 0) + (parseFloat(cs0.borderBottomWidth) || 0)
  const borderBoxH = el.getBoundingClientRect().height
  const contentH = borderBoxH - padTB - borderTB
  ...
  while (node && node.nodeType === 1 && node !== root) {
    ...
    const isFlexItem =
      !!parent && (parentDisplay.includes('flex') || parentDisplay.includes('grid'))
    ...
    const entry = {
      ...
      lineHeightPx: resolveLineHeightPx(cs, node),
      layoutLineBoxPx: measureLayoutLineBoxPx(cs, node),
      borderBoxH: node.getBoundingClientRect().height,
      flexItem: isFlexItem,
      alignSelf: cs.alignSelf,
      effectiveCrossAlign: isFlexItem ? debugEffectiveCrossAlign(node, cs) : null,
      crossStretchItem: debugIsFlexCrossStretchItem(node, cs),
    }
    const cls =
    ...
    else if (cls) entry.classHint = cls

    if ((cs.display || '').includes('flex')) {
      entry.flexDirection = cs.flexDirection
      entry.alignItems = cs.alignItems
      entry.justifyContent = cs.justifyContent
      entry.gap = cs.gap
    }
    if (isFlexItem && pcs) {
      entry.parentDisplay = parentDisplay
      entry.parentFlexDirection = pcs.flexDirection
      entry.parentAlignItems = pcs.alignItems
      entry.parentGap = pcs.gap
      entry.flexGrow = cs.flexGrow
      entry.flexShrink = cs.flexShrink
      entry.flexBasis = cs.flexBasis
    }
    chain.push(entry)
    node = parent
  }
  ...
  return {
    chain,
    summary: {
      crossStretchItem: debugIsFlexCrossStretchItem(el, cs0),
      effectiveCrossAlign: debugEffectiveCrossAlign(el, cs0),
      lineHeightPx: lineHeightPx0,
      layoutLineBoxPx: layoutLh0,
      borderBoxH,
      contentH,
      boxHeight: box0.height,
      borderBoxMinusLinePx:
        layoutLh0 != null ? borderBoxH - padTB - borderTB - layoutLh0 : null,
      inkVsBorderTop: ink ? ink.top - box0.top : null,
      capVsBorderTop: cap?.topInBorder ?? null,
      halfLeading: cap?.halfLeading ?? null,
    },
  }
}
...
/**
 * Per-row ink density profile inside a canvas region (debug only).
 * @param {HTMLCanvasElement} canvas
 * @param {{ x: number, y: number, w: number, h: number }} region device px
 * @param {{ minRowCoverage?: number, lumMax?: number, minAlpha?: number }} [opts]
 */
export function measureCanvasInkRowProfile(canvas, region, opts = {}) {
  const { x, y, w, h } = region
  const minRowCoverage = opts.minRowCoverage ?? 0.02
  const lumMax = opts.lumMax ?? 170
  const minAlpha = opts.minAlpha ?? 32
  ...
  for (let row = 0; row < h; row++) {
    const density = rowInkDensity(data, w, row, colStart, colEnd, lumMax, minAlpha)
    profile.push(density)
    if (firstInkRow == null && density >= minRowCoverage) firstInkRow = row
  }
  const peak = profile.length ? Math.max(...profile) : 0
  return {
    profile,
    firstInkRow,
    firstInkRowSub: firstInkRow == null ? null : subpixelInkEdgeFromTop(profile, minRowCoverage),
    minRowCoverage,
    peakDensity: peak,
    region: { x, y, w, h },
  }
...
export function measureCanvasInkForElement(canvas, root, el, dpr = 1) {
  ...
  // LTR labels: fixed-width band from the left edge (not full element width) so
  // short vs long words use the same scan geometry for cap-top detection.
...
export function compareCheckoutStructure(liveRoot, svgStr, tolPx = 0) {
  const { iframe, cloneRoot } = mountSvgClone(svgStr)
  /** @type {{ title: string, kind: 'text'|'input', liveEl?: Element, cloneEl?: Element, liveMetrics: object, cloneMetrics: object, liveFlexDebug?: object|null, cloneFlexDebug?: object|null, siblingGap?: { live: number, clone: number }|null, usedHeight?: { live: string, clone: string }|null }[]} */
  const sections = []
  ...
  function addSection(title, liveEl, cloneEl, opts = {}) {
    ...
    const liveFlexDebug =
      kind === 'text' ? collectFlexLayoutDebug(liveEl, liveRoot) : null
    const cloneFlexDebug =
      kind === 'text' && cloneRoot
        ? collectFlexLayoutDebug(cloneEl, cloneRoot)
        : null
    let siblingGap = null
    if (opts.gapToInput) {
      const liveInput = liveEl.parentElement?.querySelector('input:not([type=checkbox])')
